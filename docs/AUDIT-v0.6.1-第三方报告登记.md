...
1. **`catch {}` 里还包着写操作的位置**：`ledger.js`（§3.17）、`task-state.js`（§3.18）
   **已修**；`session-index.js` / `titles.js` / `compact.js` / `cachestats.js` **未修**，
   丢失的是会话索引、标题与计费明细。下一批做。
   （`compact.js` 的两处 `catch {}` 已复核为**正常**：它是「结构化摘要失败→退回纯文本」的
   重试写法，两次都失败会返回 null 由调用方处理，不属于静默吞。）
