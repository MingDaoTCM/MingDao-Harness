...
终端注入（§3.1 P2-5）、项目记忆持久化注入（§3.2 P2-9）。

**剩余（按建议顺序）**：

1. **静默失效类收尾**：代码审计报告 P2-5（`mcp-presets` 的 `{dir}` 未替换 → MCP 启动即失败且无提示）、
   P2-12（`repl.js` 自动标题缺 try/catch，`cli.js` 已加过同类保护）；
2. **日志/写入类**（代码审计报告 P2-1/P2-2/P2-3）：`removeMemoryLines` 尾换行、
   `audit.js` 截断依赖进程内计数（CLI 下是死代码，日志无界增长）、截断用非原子写；
3. **调度/进程类**：自评 P2-6（`sleeperAlive` 裸 `process.kill` 无归属校验）、
   P2-10（`killTask` 只发 SIGTERM）、P2-11（`runOnce` 轮询期不检查租约）、
   代码审计 P2-9（声明式工具超时不清理进程组）；
4. **锁**：自评 P2-7（`Atomics.wait` 阻塞事件循环 + `timeoutMs < staleMs` 死区）；
5. **能力声明与实现不一致**：自评 P2-1（`pack.json` 的 `permissions` 纯声明不生效）、
   P2-2（deny 链式绕过）、P2-3（git 前缀缩写绕过）——这三条要么补实现、要么把措辞改到与实现一致；
6. **重复实现收敛**：代码审计 P2-11（只读工具集合两份）、自评 P3-4 的同类（时间口径三处、追加截断三份）；
7. 其余 P3 与 §4.3（`audit-report.md` 口径不同，需先提取它自己的 issue-index 再排期）。
