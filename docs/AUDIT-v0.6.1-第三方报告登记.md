...
| B-CON-1 | 安全/注入 | ReDoS | ⚠ 待核实（未指明是哪个正则） |
| B-HK-1 | 安全/注入 | `shell:true`（`hooks.js`） | ⚠ 待核实（hook 命令来自用户自己的配置，风险面待评估） |
| B-SR-1 | 安全/注入 | 路径穿越 | ⚠ 待核实（未指明位置） |
| A-PI-1 | 安全 | 「无 Key 强制」 | ✅ v0.6.0 C4 已处理（本地/内网端点免 Key） |
| A-LG-1 | 数据完整性 | 哈希链盲区 | ⚠ 待核实（v0.6.0 C1 的 `verifyRun` 能发现改行与删行；是否仍有盲区需具体化） |
| B-CS-1 | 数据完整性 | 竞态丢数据 | ⚠ 待核实 |
| B-WS-1/2 | 数据完整性 | 静默吞写入 | ⚠ 待核实（本仓多处 `catch {}`，需定位具体位置） |
