...
1. ~~**`catch {}` 里还包着写操作的位置**~~ ✅ **已清完**：`ledger.js`（§3.17）、
   `task-state.js`（§3.18）、`cachestats.js` + `session-index.js`（§3.19）**已修**；
   `titles.js` 复核为**非缺陷**（调用点已判空）、`compact.js` 复核为**正常重试写法**
   （两次都失败会返回 null 交由调用方处理）。**这一类不再是"部分修"**。
