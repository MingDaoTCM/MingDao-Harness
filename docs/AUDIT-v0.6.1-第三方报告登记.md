...
2. ~~**`withFileLockSync` 的阻塞面**（自评 P2-7）~~ 🟡 **已修一半**（§3.26）：新增异步版
   `withFileLock`，**WebUI 请求路径**上的 `workspace`（7 处）已迁移，请求不再冻结事件循环；
   其余 16 处（schedule 12 / tasks 2 / cachestats 1 / sync 1）**仍用同步版**，
   原因与影响已在 §3.26 逐条说明——**这是完整的现状，不是"差不多做完了"**。
