...
export function notify(/** @type {any} */ title, /** @type {any} */ message) {
  try {
    ...
    if (process.platform === 'linux') {
      const child = spawn('notify-send', [t, text], spawnOpts({ detached: true, stdio: 'ignore' }));
      child.on('error', () => {});
      child.unref();
    } else if (process.platform === 'darwin') {
      // 质检 A8：osascript 逐参数传递（spawn 不经 shell，无需手工引号转义）
      const child = spawn('osascript', ['-e', 'display notification ' + JSON.stringify(text) + ' with title ' + JSON.stringify(t)], spawnOpts({ detached: true, stdio: 'ignore' }));
      child.on('error', () => {});
      child.unref();
      ...
      const child = spawn('powershell', ['-NoProfile', '-NonInteractive', '-EncodedCommand', enc], spawnOpts({ detached: true, stdio: 'ignore' }));
      child.on('error', () => {});
      child.unref();
    }
  } catch {
